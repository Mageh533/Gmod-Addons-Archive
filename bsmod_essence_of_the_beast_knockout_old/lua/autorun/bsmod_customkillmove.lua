...
--IMPORTANT: Make sure to change the EssenceOfTheBeastKnockout to something else to avoid conflicts with other custom killmove addons
hook.Add("CustomKillMoves", "EssenceOfTheBeastKnockout", function(ply, target, angleAround)
	...
	plyKMModel = "models/weapons/c_limbs_yakuza01.mdl" --We set the Players killmove model to the custom one that has the animations
	...
	if (target:GetClass() == "npc_zombie" or target:IsPlayer() == true or target:GetClass() == "npc_magnusson" or target:GetClass() == "npc_gman" or target:GetClass() == "npc_eli" or target:GetClass() == "npc_barney" or target:GetClass() == "npc_kleiner" or target:GetClass() == "npc_mossman" or target:GetClass() == "npc_alyx" or target:GetClass() == "npc_monk" or target:GetClass() == "npc_citizen" or target:GetClass() == "npc_combine" or target:GetClass() == "npc_metropolice") and ply:OnGround() and target:LookupBone("ValveBiped.Bip01_Spine") and (angleAround > 135 and angleAround <= 225) and math.random(1, 3) >= 2 then
	
		targetKMModel = "models/bsmodimations_yakuza01target.mdl" --Set the Targets killmove model
		
		if math.random(1, 2) == 1 then
			animName = "killmove_human_yakuzathrow" --Set the name of the animation that will play for both the Player and Target model
		end
	end
	
	--Positioning the Player for different killmove animations
	if animName == "killmove_human_yakuzathrow" then
		plyKMPosition = target:GetPos() + (target:GetForward() * -50 ) --Position the player in front of the Target and x distance away
	end
...
hook.Add("CustomKMEffects", "EssenceOfTheBeastKnockout", function(ply, animName, targetModel)
	
	local targetHeadBone = nil
		
	if IsValid (targetModel) then targetHeadBone = targetModel:GetHeadBone() end
	
	if animName == "killmove_human_yakuzathrow" then --Check the killmove animation names
	...
	elseif animName == "killmove_human_yakuzathrow" then
