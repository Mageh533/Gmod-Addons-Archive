...
function ENT:Initialize()
	if IsValid(self:GetOwner()) then
		if self:GetOwner():GetModel() == self:GetModel() then
			self:GetOwner().kmviewentity = self
		else
			self:GetOwner().kmviewanim = self
		end
		
		if self:GetOwner().GetPlayerColor != nil then
			local playerColor = nil
			playerColor = self:GetOwner():GetPlayerColor()
			
			if playerColor != nil then
				self.GetPlayerColor = function() return Vector( playerColor.r, playerColor.g, playerColor.b ) end
			end
		end
	end
end
