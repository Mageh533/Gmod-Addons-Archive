...
--IMPORTANT: Make sure to change the TigerDrop to something else to avoid conflicts with other custom killmove addons
hook.Add("CustomKillMoves", "TigerDrop", function(ply, target, angleAround)
	...
	plyKMModel = "models/weapons/c_limbs_yakuzaplayertigerdrop.mdl" --We set the Players killmove model to the custom one that has the animations
	...
	if math.random(1, 2) >= 2 and ply:OnGround() and not string.find(target:GetClass(), 'zombie') and target:GetClass() ~= 'npc_zombine' and target:LookupBone("ValveBiped.Bip01_Spine") and (angleAround <= 45 or angleAround > 315) then
	
		targetKMModel = "models/bsmodimations_target_yakuzatigerdrop.mdl" --Set the Targets killmove model
	...
	if animName == "killmove_tigerdrop" then
		plyKMPosition = target:GetPos() + (target:GetForward() * 72 ) --Position the player in front of the Target and x distance away
...
hook.Add("CustomKMEffects", "TigerDrop", function(ply, animName, targetModel)
	...
	elseif animName == "killmove_tigerdrop" then
